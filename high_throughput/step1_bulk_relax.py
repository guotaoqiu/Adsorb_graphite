...
from utils.incar_generator import (
    generate_incar, write_incar, read_species_from_poscar, MP_POTCAR_MAP
)
...
def generate_potcar(species, potcar_dir, output_path):
    """Concatenate POTCAR files for each species."""
    with open(output_path, 'w') as fout:
        for sp in species:
            variant = MP_POTCAR_MAP.get(sp, sp)
            ...
            if not os.path.exists(potcar_path):
                print(f"    WARNING: POTCAR not found for {sp} ({variant})")
                return False
...
def setup_bulk_calculations(input_dir, work_dir, potcar_dir=None,
                            partition='cu', ntasks=64):
    ...
    for vasp_path in vasp_files:
        ...
        # Generate POTCAR
        if potcar_dir:
            success = generate_potcar(species, potcar_dir,
                                       os.path.join(calc_dir, 'POTCAR'))
            if not success:
                print(f"  {compound}: POTCAR generation failed")
...
def main():
    ...
    p_setup.add_argument('--potcar_dir', default=None, help="POTCAR library directory")
    ...
    if args.command == 'setup':
        setup_bulk_calculations(args.input_dir, args.work_dir, args.potcar_dir)
